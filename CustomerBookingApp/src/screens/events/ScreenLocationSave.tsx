...
const ScreenLocationSave = ({navigation, route}: any) => {
  // const {
  //   onSelect,
  // }: {
  //   onSelect: (val: LocationModelSuggest) => void;
  ...
  const onSelect = navigation
    .getParent()
    ?.getState()
    ?.routes?.find((route: any) => route.name === 'Add')
    ?.state?.routes?.find((route: any) => route.name === 'AddNewEventScreen')
    ?.params?.onSelect;

  if (onSelect) {
    onSelect('Hello từ ScreenB!');
  } else {
